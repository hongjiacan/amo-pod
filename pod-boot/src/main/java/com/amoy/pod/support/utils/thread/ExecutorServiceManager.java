package com.amoy.pod.support.utils.thread;
...
public class ExecutorServiceManager {
     ...
     */
    public static void shutdownAndAwaitTermination(ExecutorService exec, long awaitTime) throws RuntimeException {
        exec.shutdown();
        try {
            //等待任务全部完成或达到指定时间
            if(!exec.awaitTermination(awaitTime, TimeUnit.SECONDS)){
                ...
                if(!exec.awaitTermination(60, TimeUnit.SECONDS)){
                    throw new RuntimeException("exec did not terminate");
                }
            }
